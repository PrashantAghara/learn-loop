...
def send_message(payload: MessageRequest, user_id: str = Depends(get_current_user_id)):
    logger.info("Processing message request", extra={"user_id": user_id, "message_length": len(payload.message)})
    ...
    logger.info("Message processing complete", extra={"user_id": user_id, "intent": result.get("intent"), "topic": result.get("topic")})
...
def send_reaction(
    ...
    logger.info("Recording reaction", extra={"user_id": user_id, "topic": payload.topic, "reaction": payload.reaction})
