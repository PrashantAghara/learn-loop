...
async def research_topic(query: str, max_results: int = 5) -> list[Paper]:
    ...
    papers = (
        await search_arxiv(query, max_results)
        + await search_openalex(query, max_results)
        + await search_semantic_scholar(query, max_results)
        + await search_wikipedia(query, max_results)
        + await search_tavily(query, max_results)
    )
    seen: set[str] = set()
    deduped: list[Paper] = []
    for p in papers:
